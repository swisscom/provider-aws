...
func (c *customConnector) Connect(ctx context.Context, cr *svcapitypes.ProvisionedProduct) (managed.TypedExternalClient[*svcapitypes.ProvisionedProduct], error) {
	sess, err := awsclient.GetConfigV1(ctx, c.kube, cr, cr.Spec.ForProvider.Region)
	if err != nil {
		return nil, errors.Wrap(err, errCreateSession)
	}

	awsCfg, err := awsclient.GetConfig(ctx, c.kube, cr, cr.Spec.ForProvider.Region)
	if err != nil {
		return nil, errors.Wrap(err, errCreateSession)
	}
...
func SetupProvisionedProduct(mgr ctrl.Manager, o controller.Options) error {
	...
	reconcilerOpts := []managed.ReconcilerOption{
		...
		managed.WithTypedExternalConnector(&customConnector{kube: mgr.GetClient(), options: o}),
