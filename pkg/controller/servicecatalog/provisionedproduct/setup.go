...
func (c *customConnector) Connect(ctx context.Context, mg cpresource.Managed) (managed.ExternalClient, error) {
	...
	if c.options.Features.Enabled(features.EnableReconciliationMetrics) {
		cust.metrics.enabled = c.options.Features.Enabled(features.EnableReconciliationMetrics)
