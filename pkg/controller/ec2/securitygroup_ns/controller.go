...
func SetupSecurityGroup(mgr ctrl.Manager, o controller.Options) error {
	name := managed.ControllerName(nsapitypes.SecurityGroupGroupKind)
	...
	r := managed.NewReconciler(mgr,
		resource.ManagedKind(nsapitypes.SecurityGroupGroupVersionKind),
		reconcilerOpts...)
...
func (c *connector) Connect(ctx context.Context, mg resource.Managed) (managed.ExternalClient, error) {
	cr, ok := mg.(*nsapitypes.SecurityGroup)
	if !ok {
...
func (e *external) Observe(ctx context.Context, mgd resource.Managed) (managed.ExternalObservation, error) {
	cr, ok := mgd.(*nsapitypes.SecurityGroup)
	if !ok {
