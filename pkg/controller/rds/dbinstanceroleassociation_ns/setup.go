package dbinstanceroleassociation_ns
...
func SetupDBInstanceRoleAssociation(mgr ctrl.Manager, o controller.Options) error {
	name := managed.ControllerName(nsapitypes.DBInstanceRoleAssociationGroupKind)
	opts := []option{
		func(e *external) {
	...
	r := managed.NewReconciler(mgr,
		cpresource.ManagedKind(nsapitypes.DBInstanceRoleAssociationGroupVersionKind),
		reconcilerOpts...)

	return ctrl.NewControllerManagedBy(mgr).
		Named(name).
		WithOptions(o.ForControllerRuntime()).
		For(&nsapitypes.DBInstanceRoleAssociation{}).
		WithEventFilter(cpresource.DesiredStateChanged()).
		Complete(r)
}

// GenerateDescribeDBInstancesInput returns the input for the read operation
func GenerateDescribeDBInstancesInput(cr *nsapitypes.DBInstanceRoleAssociation) *svcsdk.DescribeDBInstancesInput {
	return &svcsdk.DescribeDBInstancesInput{
		DBInstanceIdentifier: cr.Spec.ForProvider.DBInstanceIdentifier,
	}
}

func preCreate(_ context.Context, cr *nsapitypes.DBInstanceRoleAssociation, input *svcsdk.AddRoleToDBInstanceInput) error {
	input.DBInstanceIdentifier = cr.Spec.ForProvider.DBInstanceIdentifier
	input.RoleArn = cr.Spec.ForProvider.RoleARN
	return nil
}

func preDelete(_ context.Context, cr *nsapitypes.DBInstanceRoleAssociation, input *svcsdk.RemoveRoleFromDBInstanceInput) (bool, error) {
	input.DBInstanceIdentifier = cr.Spec.ForProvider.DBInstanceIdentifier
	input.RoleArn = cr.Spec.ForProvider.RoleARN
	return input.RoleArn == nil || input.DBInstanceIdentifier == nil, nil
}

func (e *external) observer(ctx context.Context, cr *nsapitypes.DBInstanceRoleAssociation) (managed.ExternalObservation, error) {
